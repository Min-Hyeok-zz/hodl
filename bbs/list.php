...
if (!$sst) {
    ...
    $sst = preg_match("/^(wr_datetime|wr_hit|wr_good|wr_nogood|wr_good10)$/i", $sst) ? $sst : "";
}
...
if ($is_search_bbs) {
    ...
    if ($sst == 'wr_good10') { //추천수가 10개 이상인 게시글만 날짜순으로 desc(내림차순) 정렬
        $sql = " select * from {$write_table} where wr_good >= 10";
        $sql .= " order by wr_datetime desc";
...
if ($sst == "wr_good10") {
    $a = sql_query($sql);
    $total_page = ceil($a->num_rows / $page_rows);
}

if ($page > $total_page+1) alert("잘못된 접근입니다.");
